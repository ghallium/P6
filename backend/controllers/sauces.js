    ...
    .then((sauce) => {
      if (sauce.userId != req.auth.userId) {
        ...
        fs.unlink(`images/${filename}`, () => {
          // Suppression fichier avec unlink
          sauce
...
exports.getOneSauce = (req, res, next) => {
  // Appelle une sauce avec "Request", "Result" et "Next" (prochain controller)
...
exports.getAllSauces = (req, res, next) => {
  // Appelle toutes les sauces avec "Request", "Result" et "Next" (prochain controller)
...
/*
      ...
      if (req.body.like == 0) {
        ...
        if (sauce.usersLiked.indexOf(req.body.userId) != 1) {
          ...
          sauce.usersDisliked.splice(
            ...
            1
          ); // Suppression du dislike par rapport à son id
        }
        sauce.save();
      }
      ...
      res
        .status(200)
        .json({ message: "Votre like/dislike a bien été pris en compte !" });
    })
    // réponse échec
    .catch((error) => {
      res.status(500).json({ error });
